...
22. **Named local studio:** Dave bookmarks `dln.local` names (Debian Caddy :80). Cookie `Domain=.dln.local` on those hosts. Live `/desk` tunnels the home book. Clock live overlay. Houses sheet (View site / Open live / View EPK) on the book and on the lab `/links`. **Assets is the house library** (dropdowns, drag-and-drop, text notes, comments, download, delete uploads, tick share). **ModYu journalist kit is Dave’s pages** (not a scroller): campaigns including About Ann-Marie; Download PDF per page with logo+banner on the PDF foot only. **ModYu and DAA text is in the hub**. Various Titles later. **EPKs pull only shared assets** into the MAP editor for other houses. `/epk` is the in-house press-kit gate. Each kit is unique at `/epk/{kit}`. A cookie for one house never opens another. DAA kit is fed from that house’s `_meta/assets/daa.json` (slim copy in the hub). Lab launches View site / View EPK; no harvest box in compose. Ordinary Send does not deploy. **Board stays off the live ship.**
