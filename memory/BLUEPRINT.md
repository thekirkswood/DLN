...
| `/epk/[kit]` | EPK cookie **for this kit**, studio, or tagged client | **Unique kit.** Centre of the top bar is **DLN** (this pack is a Design Lab North product). House name stays left. ModYu landing + side menu follow Dave’s HT4 MAP (two groups with blurbs; hero plate, boilerplate, story, market, story tiles, vault plate). ModYu founder still is **Ann-Marie Barlow** from the DLN book (`kit-stills/modyu/ann-marie-barlow.jpg`); `founder-headshot.png` is aftercare product, not her. On-screen pages are web articles, not PDF facsimiles. **Download PDF** on desk stories only — logo and banner on the PDF foot. Chamfered Paper/Ink, not the PDF’s rounded boxes. Other kits stay on the HT4 MAP chrome (`_meta/HT4_EPK MAP.pdf`): cover heroes, story plates, vault. Live pack **pulls only assets ticked to share**. Unique seed copy in `epk-docs/{kit}.ts`. Kits: dln, modyu, pfp, dks, swarm, daa, titles. Cookie mismatch **gates in place**. Do not put DAA on the greenhouse wall. |
...
- Cookie: `dln_session` httpOnly. Live Domain `.designlabnorth.com`. Named LAN Domain `.dln.local` plus a host-only twin so a rejected Domain still sticks on this host. Host-only on localhost and on the IP backup. `Secure` on HTTPS (from `X-Forwarded-Proto`, not a blanket flag). Path `/`. **90 days**. Read **every** `dln_session` on the request (Next’s `cookies().get` collapses duplicates and was wiping a good session with a stale twin). Middleware only refreshes when those values agree. `/api/auth/me` rewrites both cookies to the token that actually signed in, and upgrades a studio book token to a home ticket when the key is present. Press kit cookie `dln_epk` is the kit id (30 days), same domain rules. Signed-in studio or tagged client visiting a kit stamps that cookie so they do not type a code. Cross-host studio walk (campus → live kit, LAN twins) goes through `/api/auth/lan-enter` → consume. Home tickets POST across hosts so campus and live do not need a shared `lan-handoff.json`. One-time codes remain for LAN when a ticket cannot be minted. Lab without a cookie bounces through hub `/api/auth/lan-enter` (copy, do not dump `next=builder.dln.local` onto `/account`). Root layout is always dynamic — do not cache a signed-out header. Logout and other server redirects use `DLN_PUBLIC_URL` / forwarded host, never `0.0.0.0:3000`. Client books stay on their host — do not copy Ann-Marie’s VPS session onto campus or onto `modyu.designlabnorth.com`.
