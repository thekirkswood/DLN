...
| Path / host | Who | What |
|---|---|---|
| `designlabnorth.com` `/` | public | Small Design Lab North. Kicker then Dave’s name copy as stacked sections (h1 opening line, same-size mid-grey name sentence, h2 Design / Lab / North — site heading sizes, not a three-column row). Then three offer columns on the same measure as the intro (42rem) when they lock; one full centred slide with a swipe when they cannot. All three home columns are Dave’s lists (Design: Logos, Brand Identity Systems, UI, Design for Print, Packaging. Strategy: Brand Strategy, Marketing Strategy, Online Strategy, Start-up Strategy, Brand Audits, Over-arching Strategic Consultancy. Websites: Website Builds, Website Modelling, New sites, Rebuilds, Facelifts, Live hosts). Trial paragraphs and Dave’s institute copy stay cached. Column links: Contact our consultants, Contact Design, Contact the web team. No form in the body. Header: mark, Home, Practice, Work, Greenhouse, Sign in or an account **circle**. Methodology lives in the footer, not the main menu. Public nav stays the public nav — Lab / Builder do not appear here. Studio open Campus from under the circle. Menu is initial caps, not all caps. |
...
| `/method` | public | **Methodology.** Seven RUUN papers at the top (moved off home). How Design Lab North work. Direct pitch (peers, same method at every scale with type that grows). Quality filter and campus pipeline as diagrams; idle play lights the tile under the token. Phone: stacked rooms, left-rail token, compressed copy under each tile lighting with it. Desktop facets unchanged. Various Titles close with the locked VT plates. In the footer as Methodology. Not in the main menu. |
