...
| `builder.dln.local` | studio | Named lab. Change / plan / note. House rail should tab like the campus plus-select (quote `memory/builder-site-tabs.md`). Offline window = localUrl / IP:port and wake if off; do not iframe public subdomains on the LAN. **View site** and **View EPK** launch in a new tab. The lab window iframes Design Lab North and the hub kit; other houses launch rather than sit empty (they block embedding). Page chips still include Press (hub `/epk/{kit}`). `/links` is Dave’s click sheet. No harvest lecture in compose. Push to git and live + Hotfix are named buttons. Send does not deploy. Backup `:3100`. |
