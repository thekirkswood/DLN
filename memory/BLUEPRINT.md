...
| `/greenhouse` | public | Greenhouse projects. Intro: **Products we make.** No named list in the intro. Various Titles first. |
| `/greenhouse/[slug]` | public | Studio product story. Enter the product domain if one exists. |
| `/account` | cookie | Profile is **pic + name** (click pic to upload). No name field, no email on the head. Five chamfered boxes under: **Sites**, **Press kits**, **Assets**, **Payments**, **Notifications**. Those boxes are ordinary links (full page load) so a walk home → back, refresh, or box-to-box does not dump them to Sign in. Payments is rolls, invoices, receipts, sittings, Various Titles. Notifications holds due invoices, notes, what’s new. A due invoice puts a bubble on Payments and a line in Notifications. Client tiles use `?view=`. Sites: Visit site + Press kit + **Assets** (library) + **Edit kit**. Assets is the **asset hub**: house and type dropdowns, drag-and-drop upload, write a text note, comments, download, delete (uploads/docs only), tick **On the press kit**. **ModYu** holds Dave’s HT4 EPK as text documents; **DAA** holds the site pack as text. All off the kit until ticked. Edit kit is the MAP framework editor. See it live opens `/epk/{kit}`. Studio tiles open Houses / EPKs / Assets / Pay / Notifications; the book row under is Clients / Onboarding / Book / Settings / Watch / Clock. Unauth → `/login`. |
...
| `/epk/[kit]` | EPK cookie **for this kit**, studio, or tagged client | **Unique kit.** Other kits: centre of the top bar is **DLN** (the pack is a Design Lab North product), house name left. **ModYu** is different: top left is the ModYu mark plus “ModYu electronic press kit”; top middle is **HT4** (clickable, its own page); the side box opens with **HT4**, not “electronic press kit”. HT4’s page is the live ModYu `/ht4` explorer: coloured phase frames, matching stills, and the panel that opens underneath (picture, copy, ingredients, how to use). No reviews, no Buy now. A copyable **system summary** sits between the stage explorer and the Evidence / What we claim / Our story reads. Rail groups have names only: no studio asides. Landing still has hero plate, boilerplate, story, market, story tiles, vault plate. Founder still is **Ann-Marie Barlow** from the DLN book (`kit-stills/modyu/ann-marie-barlow.jpg`); `founder-headshot.png` is aftercare product, not her. On-screen pages are web articles, not PDF facsimiles. **Download PDF** on desk stories only — logo and banner on the PDF foot. Footer: Design Lab North hosted. Chamfered Paper/Ink, not the PDF’s rounded boxes. Other kits stay on the HT4 MAP chrome (`_meta/HT4_EPK MAP.pdf`): cover heroes, story plates, vault. Live pack **pulls only assets ticked to share**. Unique seed copy in `epk-docs/{kit}.ts`. Kits: dln, modyu, pfp, dks, swarm, daa, titles. Cookie mismatch **gates in place**. Do not put DAA on the greenhouse wall. |
...
| `/preview/[slug]` | cookie | Old door. Campus home no longer links it. Redirects to the live plot URL if one exists. |
...
| `daa.designlabnorth.com` | public | Digital Adoption Advisor (`plot-daa`). Ungated. Own site, not a redirect to the hub. Campus window on live is this host. LAN `daa.dln.local` / `:3050`. |
...
| `/login` `/logout` | public / session | Cookie `dln_session`. Header Sign in → `/account`. Studio addresses `@designlabnorth.com`. **One cookie per host** — never a Domain + host-only twin. Named LAN (`dln.local`) is host-only (`.local` often rejects `Domain=.dln.local`). Live uses `DLN_COOKIE_DOMAIN=.designlabnorth.com`. Setting a session expires any leftover twin. `Secure` follows HTTPS. Sign in `next` may be a LAN lab URL or the public hub. Studio session copies via `/api/auth/lan-enter` → `/api/auth/lan-consume`: home ticket POST when possible, else a half-hour one-time code on the same disk. Studio login ok/fail and lastLogin are Clock facts (no passwords). |
...
| `builder.dln.local` | studio | Named lab. Change / plan / note. House rail should tab like the campus plus-select (quote `memory/builder-site-tabs.md`, paste `memory/builder-paste.md`). Offline window = localUrl / IP:port **at `/`**, never `/go/{slug}`. Do not iframe public subdomains on the LAN. **View site** and **View EPK** launch in a new tab. The lab window iframes Design Lab North and the hub kit; other houses launch rather than sit empty (they block embedding). Page chips still include Press (hub `/epk/{kit}`). `/links` is Dave’s click sheet. No harvest lecture in compose. Push to git and live + Hotfix are named buttons. Send does not deploy. Backup `:3100`. Campus leftover `/lab` on a lab host **302s here**. Live `designlabnorth.com/lab` stays 404. |
...
- Cookie: `dln_session` httpOnly. **One value per host.** Live Domain `.designlabnorth.com`. Named LAN and localhost and the IP backup are **host-only** (do not set `Domain=.dln.local` — special-use TLD, rejected, then a twin fights the live cookie). `Secure` on HTTPS (from `X-Forwarded-Proto`, not a blanket flag). Path `/`. **90 days**. Login, logout, lan-consume, and `/api/auth/me` write the cookie and expire any leftover twin. Middleware does **not** Set-Cookie — rewriting on every click was recreating the fight. Read every `dln_session` still on the request until old twins die (Next’s `cookies().get` collapses duplicates). `/api/auth/me` refreshes the live cookie and upgrades a studio book token to a home ticket when the key is present. Press kit cookie `dln_epk` is the kit id (30 days), same one-cookie rule. Signed-in studio or tagged client visiting a kit stamps that cookie so they do not type a code. Cross-host studio walk (campus → live kit, named LAN hub and lab only) goes through `/api/auth/lan-enter` → consume. **Only Design Lab North (`:3010` / `dln.local`) and Builder (`:3100` / `builder.dln.local`) consume the ticket.** Posting it at ModYu, DAA, or another house origin is their 404 page. The lab at `:3100` **POSTs the home ticket** onto `/api/auth/lan-consume` and verifies it — a GET-only handoff that ignored `dln1.` tokens looped that URL. Home tickets POST across hosts so campus and live do not need a shared `lan-handoff.json`. One-time codes remain for LAN when a ticket cannot be minted. Lab without a cookie bounces through hub `/api/auth/lan-enter` (copy, do not dump `next=builder.dln.local` onto `/account`). Root layout is always dynamic — do not cache a signed-out header. Logout and other server redirects use `DLN_PUBLIC_URL` / forwarded host, never `0.0.0.0:3000`. Client books stay on their host — do not copy Ann-Marie’s VPS session onto campus or onto `modyu.designlabnorth.com`.
...
| Plots | One compose service per **hosted** plot. ModYu at `modyu.designlabnorth.com`. Paul Fosbury Portraits at `paulfosbury.designlabnorth.com` and `paulfosburyportraits.com` (`plot-pfp`). DAA at `daa.designlabnorth.com` (`plot-daa`). Swarm growing copy at `swarmfund.designlabnorth.com`. Various Titles at `varioustitles.com` (`plot-titles`). |
...
/srv/dln/
  ...
  plots/daa      Digital Adoption Advisor source bind (`plot-daa`)
...
| Client | ModYu, Paul Fosbury Portraits, Digital Adoption Advisor. More to come. |
