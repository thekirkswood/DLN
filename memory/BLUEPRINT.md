...
| Path / host | Who | What |
|---|---|---|
| `designlabnorth.com` `/` | public | Small Design Lab North. Kicker then Dave’s name copy (h1 opening line, same-size mid-grey name sentence). Design / Lab / North is a three-step concertina: light-grey arrow right to open, up to close, mid grey when open and on hover, each independent, stays open until closed. Tight leading like the title. Top and bottom keylines only, hairline and very light. Equal gap above the concertina and from its lower keyline to Strategy / Design / Websites. Looping six-slide gif under all that copy, full screen width (same bleed as Work). Any leftover strip above the footer takes the current slide’s ground; the footer stays Paper. Strategy / Design / Websites are always open (not fold buttons), ink titles. Same 42rem measure as North, centred at every width. Three columns when they fit; one open column you swipe when they cannot, with dots not slider rules. Same lists and CTAs. No form in the body. Header: mark, Home, Practice, Work, Greenhouse, Sign in or an account **circle**. Footer (every public page except Work): Design Lab North and build@ left; BKND (smaller, bold) and ground circles centred; Methodology, Privacy, Terms as a left-ranged block aligned to the right end of the measure. Initial caps, email lowercase. Methodology is not in the main menu. Public nav stays the public nav — Lab / Builder do not appear here. Studio open Campus from under the circle. Menu is initial caps, not all caps. |
| `/design` `/strategy` `/build` | public | The offer. `/design`, `/strategy`, and `/build` match: Dave’s body, through-list, Let’s Establish Your Scope, open Contact. `/build` title is Website Builds and hosting. Path stays `/build`. Form lands on the studio desk. |
| `/practice` | public | About us. Heading: High-Value Brand Ecosystems. Mid-grey line: Plan, Design, Build, Maintain. Dave’s practice story first, then Dave and Ewan stacked bios (1px mid-grey rules; Dave’s portrait then text), then Selected Experience and the client list. No enquire form. |
...
| `/account` | cookie | Client: profile, sites, notes, shipped patch notes, invoices. Studio: profile and a door to campus at home. The book is not copied onto the VPS. Unauth → `/login`. |
...
| `/lab` | studio, this PC (`localhost` or its LAN address on `:3010`) | Campus hub. Unit doors, then **Accounts**: Clients / Onboarding / Book / Pay / Settings as rooms that never go away. Serving is the client on the desk. Pay composes for that person. Each room has a page identifier well — same Send as `/admin`, tagged with that room (and who we are serving). Lands on the campus inbox. Header is the mark plus the account circle — Campus is the only admin surface (Builder lives as a door on campus). Public Home / Method / Practice / Greenhouse stay off this bar. 404 on the public host. Dave and Ewan both enter. |
...
| `/admin` | studio, this PC | **Campus building site** — queue for this Design Lab North Cursor (`_meta/lab-inbox/`). Send carries a page identifier (Campus, rooms, public pages). Same field as the room wells and as a client live-host suggestion. Not a public CMS. |
...
On a lab host the book is **campus**, under the unit doors (`/lab`). `/account` is Ewan and Dave: profile and sign out. On the public VPS the desk is not copied; studio `/lab` talks home through the tunnel (`ops/home-tunnel.md`). Client `/account` stays on the VPS.
